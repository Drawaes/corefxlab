...
namespace System.IO.Pipelines.Networking.Tls.Managed.Internal.Interop.Windows
{
    internal unsafe static class InteropProperties
    {
        ...
        private extern static NTResult BCryptGetProperty(IntPtr bCryptHandle, string pszProperty, out int pbOutput, int cbOutput, out int pcbResult, uint dwFlags);
        [DllImport(Dll, ExactSpelling = true, SetLastError = true, CharSet = CharSet.Unicode)]
        private extern static NTResult BCryptSetProperty(IntPtr hObject, string pszProperty, void* pbInput, int cbInput, uint dwFlags);
        [DllImport(Dll, ExactSpelling = true, SetLastError = true, CharSet = CharSet.Unicode)]
        private extern static NTResult BCryptSetProperty(IntPtr bCryptHandle, string pszProperty, ref int pbOutput, int cbOutput, uint dwFlags);
        [DllImport(Dll, ExactSpelling = true, SetLastError = true, CharSet = CharSet.Unicode)]
        private extern static NTResult BCryptGetProperty(IntPtr bCryptHandle, string pszProperty, void* pbOutput, int cbOutput, out int pcbResult, uint dwFlags);
        [DllImport(Dll, SetLastError = true, ExactSpelling = true, CharSet = CharSet.Unicode)]
        private static extern NTResult BCryptSetProperty(IntPtr hObject, string pszProperty, string pbInput, int cbInput, uint dwFlags);
        ...
        [StructLayout(LayoutKind.Sequential)]
        private struct BasicPointerArray
        {
            public int Count;
        ...
        private static string[] GetStringArrayProperty(IntPtr provider, string property)
        {
            ...
            var header = Marshal.PtrToStructure<BasicPointerArray>((IntPtr)tempBuffer);
