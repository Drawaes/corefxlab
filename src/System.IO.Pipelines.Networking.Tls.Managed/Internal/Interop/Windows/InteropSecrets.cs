...
{
    internal unsafe static class InteropSecrets
    {
        ...
        private static extern NTResult BCryptGenerateKeyPair(IntPtr hAlgorithm, out IntPtr phKey, int dwLength, int dwFlags);
        [DllImport(Dll, ExactSpelling = true, SetLastError = true, CharSet = CharSet.Unicode)]
        private static extern NTResult BCryptFinalizeKeyPair(IntPtr hKey, uint dwFlags);
        [DllImport(Dll, ExactSpelling = true, SetLastError = true, CharSet = CharSet.Unicode)]
        private static extern NTResult BCryptExportKey(IntPtr hKey, IntPtr encyrptKey, string blobType, IntPtr pbOutput, int cbOutput, out int pcbResult, uint dwFlags);
        [DllImport(Dll, ExactSpelling = true, SetLastError = true, CharSet = CharSet.Unicode)]
        private static extern NTResult BCryptImportKeyPair(IntPtr hAlgorithm, IntPtr hImportKey, string pszBlobType, out IntPtr phKey, IntPtr pbInput, int cbInput, uint dwFlags);
        [DllImport(Dll, ExactSpelling = true, SetLastError = true, CharSet = CharSet.Unicode)]
        private static extern NTResult BCryptSecretAgreement(IntPtr hPrivKey, IntPtr hPubKey, out IntPtr phSecret, uint dwFlags);
        [DllImport(Dll, ExactSpelling = true, SetLastError = true, CharSet = CharSet.Unicode)]
        private static extern NTResult BCryptDeriveKey(IntPtr hSharedSecret, string pwszKDF, void* pParameterList, IntPtr pbDerivedKey, int cbDerivedKey, out int pcbResult, uint dwFlags);
        [DllImport(Dll, ExactSpelling = true, SetLastError = true, CharSet = CharSet.Unicode)]
        private static extern NTResult BCryptDestroySecret(IntPtr hSecret);
        ...
        {
            InteropBulkEncryption.DestroyKey(key);
        }
        ...
        public static unsafe IntPtr ImportPublicKey(IntPtr provider, ReadableBuffer buffer, int keyLength)
        {
            ...
            blobHeader.dwMagic = KeyBlobMagicNumber.EchdPublic;
        ...
        public unsafe static byte[] GenerateMasterSecret12(IntPtr secret, IHashProvider hashProvider, Span<byte> clientRandom, Span<byte> serverRandom)
        {
            ...
            bufferArray[1] = new BCryptBuffer() { BufferType = BufferTypes.KDF_TLS_PRF_LABEL, cbBuffer = Tls12Utils.MasterSecretSize, pvBuffer = (void*)Tls12Utils.MasterSecretPointer };
